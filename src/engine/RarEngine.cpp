...
bool RarEngine::ExtractAll(std::string_view destPath)
{
    ...
    if (archive_read_open_filename(m_archive, m_path.c_str(), 10240) != ARCHIVE_OK)
    {
        LOG_ERR("RarEngine: failed to open archive for ExtractAll");
        return false;
    }
    ...
    while (archive_read_next_header(m_archive, &entry) == ARCHIVE_OK)
    {
        ...
        if (!out)
        {
            LOG_ERR("RarEngine: cannot create %s", fullPath.string().c_str());
            return false;
        }
        ...
        if (bytesRead < 0)
        {
            LOG_WARN("RarEngine: archive_read_data error for %s", name.c_str());
            return false;
        }
    }
